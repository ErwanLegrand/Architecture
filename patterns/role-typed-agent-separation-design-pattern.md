...
The pattern composes with the [provenance tracking pattern](/patterns/provenance-tracking-design-pattern.md): provenance labels are what role assignment enforces. A Core agent is one the type system permits to hold `Trusted` capabilities; an Edge agent is one the type system constrains to `Untrusted` outputs; a Bridge agent is one the type system permits to call declassification functions.

Because an Edge agent that reads untrusted data holds no sensitive capability and emits only `Untrusted` outputs, a prompt injection in that data cannot escalate into a sensitive action: the separation bounds the blast radius of prompt injection by construction, confining the suborned agent to the authority of its role. The same bound applies to data poisoning: an Edge agent suborned by poisoned data it has read still holds no sensitive capability, so the corruption is confined to its `Untrusted` output.
