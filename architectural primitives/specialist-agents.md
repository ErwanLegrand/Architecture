...
- **Focused context for stochastic specialists.** A stochastic specialist performs more reliably on a small, focused context than on a broad one: a scoped, per-task input reduces token consumption and inference latency and mitigates attention-dilution and lost-in-the-middle effects on long contexts. This is primarily a quality and performance benefit rather than a security property. Context isolation between agents does carry a secondary defensive effect — an attacker who suborns one agent gains no visibility into context held by sibling or upstream agents — but this is a further reason the framework prefers narrow scopes, not a primary security control.
