...
Agents exposed to external `Untrusted` data, which they transform — extract, summarize, translate, classify, parse — without holding any sensitive capability. Edge agents are the structural generalization of the Quarantined LLM (Q-LLM). The trust-position axis is the architectural expression of the [role-typed agent separation design pattern](/patterns/role-typed-agent-separation-design-pattern.md).
...
An Edge agent's outputs reach [Core agents](/primitives/core-agents.md) only after [Bridge](/primitives/bridge-agents.md) declassification; the declared topology admits no direct Edge → Core edge.
